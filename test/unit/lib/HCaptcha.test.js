...
describe('HCaptcha', () => {
  ...
  describe('verify', () => {
    ...
    // test('throws an error if HCaptcha Token set but alrady use', async (done) => {
    //   // nock('https://hcaptcha.com/')
    //   // .post('/siteverify')
    //   // .reply(200,   {
    //   //   success: false,
    //   //   'error-codes': [ 'missing-input-response', 'missing-input-secret' ]
    //   // });
    //   mockConfig.set('captcha.secret', mockHelpers.encrypt(''))
    //   const hCaptcha = new HCaptcha(mockConfig)
    //   try {
    //     await hCaptcha.verify("0xdBF251d32009372fad40684cD5EDE2B024690C05")
    //     done.fail("should have error")
    //   } catch (err) {
    //     expect(err._smErrorCode).toEqual("missing-input-response");
    //     console.log(err)
